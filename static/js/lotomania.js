// Sistema para sustentar o Girinho com a Lotomania - JavaScript
...
async function carregarCombinacaoMaisRepetida() {
    try {
        const response = await fetch('/api/lotomania/combinacao-mais-repetida');
...
async function carregarEstatisticas() {
    mostrarLoading();
    try {
        const response = await fetch('/api/lotomania/estatisticas');
        const data = await response.json();
...
function atualizarInterfaceEstatisticas() {
    ...
    maisSorteados.slice(0, 30).forEach(([numero, freq]) => {
        const badge = document.createElement('span');
        badge.className = 'number-badge';
        badge.textContent = `${numero.toString().padStart(2, '0')} (${freq}x)`;
    ...
    menosSorteados.slice(0, 30).forEach(([numero, freq]) => {
        const badge = document.createElement('span');
        badge.className = 'number-badge';
...
async function gerarJogos() {
    ...
    if (numerosFixosInput.trim()) {
        numerosFixos = numerosFixosInput.split(',').map(n => parseInt(n.trim())).filter(n => !isNaN(n) && n >= 0 && n <= 99);
    }
    
    try {
        const response = await fetch('/api/lotomania/gerar-jogos', {
            method: 'POST',
            headers: {
...
function exibirJogos(jogos) {
    ...
    jogos.forEach((jogo, index) => {
        ...
        titulo.textContent = `Jogo ${index + 1} (50 números)`;
...
async function atualizarHistorico() {
    mostrarLoading();
    try {
        const response = await fetch('/api/lotomania/atualizar-historico', {
            method: 'POST'
        });
...
function exportarJogos() {
    ...
    let texto = 'JOGOS GERADOS - LOTOMANIA\n';
    texto += '='.repeat(50) + '\n\n';
    
    jogosGerados.forEach((jogo, index) => {
        texto += `Jogo ${(index + 1).toString().padStart(2, '0')} (50 números): `;
    ...
    texto += `Quantidade de números por jogo: 50\n`;
    ...
    a.href = url;
    a.download = `lotomania_jogos_${new Date().toISOString().split('T')[0]}.txt`;
...
async function conferirJogos() {
    ...
    try {
        ...
        const response = await fetch('/api/lotomania/importar-jogos', {
            method: 'POST',
            body: formData
        });
...
function criarHTMLUltimoConcurso(dados) {
    ...
    if (acertos >= 20) classeAcertos = 'acertos-15';
    else if (acertos >= 18) classeAcertos = 'acertos-14';
    else if (acertos >= 16) classeAcertos = 'acertos-13';
    else if (acertos >= 14) classeAcertos = 'acertos-12';
    ...
    let html = `
        <div class="jogo-conferencia">
            ...
            <div>
                <p><strong>Seu jogo (50 números):</strong> ${dados.jogo.map(n => n.toString().padStart(2, '0')).join(' - ')}</p>
                <p><strong>Números sorteados (20):</strong> ${dados.numeros_sorteados.map(n => n.toString().padStart(2, '0')).join(' - ')}</p>
            </div>
    `;
    ...
    if (dados.acertos && dados.acertos.length > 0) {
        html += `
            <div class="numeros-acertados">
                <strong>Números acertados (${acertos}/20):</strong>
...
function criarHTMLHistoricoCompleto(estat) {
    ...
    let html = `
        <div class="jogo-conferencia">
            <div class="jogo-conferencia-header">
                <h5>Jogo ${estat.jogo_numero} (50 números)</h5>
            </div>
            <div>
            ...
            <div class="estatisticas-jogo">
                ...
                <p><strong>Média de acertos por concurso:</strong> ${media} de 20 números sorteados</p>
                <p><strong>Máximo de acertos:</strong> ${estat.max_acertos} de 20</p>
                <p><strong>Mínimo de acertos:</strong> ${estat.min_acertos} de 20</p>

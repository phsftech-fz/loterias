// Sistema para sustentar o Girinho com a Timemania - JavaScript
...
async function carregarCombinacaoMaisRepetida() {
    try {
        const response = await fetch('/api/timemania/combinacao-mais-repetida');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('file-import').addEventListener('change', handleFileUpload);
});

async function carregarEstatisticas() {
    mostrarLoading();
    try {
        const response = await fetch('/api/timemania/estatisticas');
        const data = await response.json();
...
function atualizarInterfaceEstatisticas() {
    ...
    maisSorteados.slice(0, 20).forEach(([numero, freq]) => {
    ...
    menosSorteados.slice(0, 20).forEach(([numero, freq]) => {
        const badge = document.createElement('span');
        badge.className = 'number-badge';
...
async function gerarJogos() {
    ...
    if (numerosFixosInput.trim()) {
        numerosFixos = numerosFixosInput.split(',').map(n => parseInt(n.trim())).filter(n => !isNaN(n) && n >= 1 && n <= 80);
    }
    
    try {
        const response = await fetch('/api/timemania/gerar-jogos', {
            method: 'POST',
            headers: {
...
function exibirJogos(jogos, timeSugerido = {}) {
    ...
    if (timeSugerido && timeSugerido.time_sugerido) {
        ...
        timeCard.className = 'jogo-card time-suggestion-card';
        ...
        if (timeSugerido.confianca !== undefined) {
            ...
            timeConfianca.className = 'time-confidence';
            ...
            if (timeSugerido.frequencia_historico !== undefined) {
                ...
                freqInfo.className = 'time-frequency';
    ...
    jogos.forEach((jogo, index) => {
        ...
        titulo.textContent = `Jogo ${index + 1} (10 números)`;
...
async function atualizarHistorico() {
    mostrarLoading();
    try {
        const response = await fetch('/api/timemania/atualizar-historico', {
            method: 'POST'
        });
...
function exportarJogos() {
    ...
    let texto = 'JOGOS GERADOS - TIMEMANIA\n';
    texto += '='.repeat(50) + '\n\n';
    
    jogosGerados.forEach((jogo, index) => {
        texto += `Jogo ${(index + 1).toString().padStart(2, '0')} (10 números): `;
    ...
    texto += `Quantidade de números por jogo: 10\n`;
    ...
    a.href = url;
    a.download = `timemania_jogos_${new Date().toISOString().split('T')[0]}.txt`;
...
async function conferirJogos() {
    ...
    try {
        ...
        const response = await fetch('/api/timemania/importar-jogos', {
            method: 'POST',
            body: formData
        });
...
function criarHTMLUltimoConcurso(dados) {
    ...
    if (acertos >= 10) classeAcertos = 'acertos-15';
    else if (acertos >= 9) classeAcertos = 'acertos-14';
    else if (acertos >= 8) classeAcertos = 'acertos-13';
    else if (acertos >= 7) classeAcertos = 'acertos-12';
    ...
    let html = `
        <div class="jogo-conferencia">
            ...
            <div>
                <p><strong>Seu jogo (10 números):</strong> ${dados.jogo.map(n => n.toString().padStart(2, '0')).join(' - ')}</p>
                <p><strong>Números sorteados:</strong> ${dados.numeros_sorteados.map(n => n.toString().padStart(2, '0')).join(' - ')}</p>
                ${dados.time_coracao ? `<p><strong>Time do Coração:</strong> ${dados.time_coracao}</p>` : ''}
            </div>
    `;
    ...
    if (dados.acertos && dados.acertos.length > 0) {
        html += `
            <div class="numeros-acertados">
                <strong>Números acertados (${acertos}/10):</strong>
...
function criarHTMLHistoricoCompleto(estat) {
    ...
    let html = `
        <div class="jogo-conferencia">
            <div class="jogo-conferencia-header">
                <h5>Jogo ${estat.jogo_numero} (10 números)</h5>
            </div>
            <div>
            ...
            <div class="estatisticas-jogo">
                ...
                <p><strong>Média de acertos por concurso:</strong> ${media} de 10 números</p>
                <p><strong>Máximo de acertos:</strong> ${estat.max_acertos} de 10</p>
                <p><strong>Mínimo de acertos:</strong> ${estat.min_acertos} de 10</p>
    ...
    numerosOrdenados.forEach(([numero, freq]) => {
        ...
        html += `<span class="freq-badge ${classe}">${numero} (${freq}x - ${percentualNum}%)</span>`;
    });
